...
public static class CreateAndFillTables
{
    ...
    {
        using var conn = new SqlConnection ("Server=" + ConfigHandler.GetSqlServerName() + ";Integrated security=SSPI;database=" + ConfigHandler.GetSqlDatabaseName());
    ...
    {
        using var dbConnection = new SqlConnection("Server=" + ConfigHandler.GetSqlServerName() + ";Integrated security=SSPI;database=" + ConfigHandler.GetSqlDatabaseName());
        dbConnection.Open();
    ...
    private static string GetCreateScript(string tableName, DataTable table)
    {
        ...
        for (var i = 0; i < table.Columns.Count; i++)
        {
            ...
            script += " nvarchar(" + (table.Columns[i].MaxLength == -1 ? "max" : table.Columns[i].MaxLength.ToString()) + ") ";
            script += ",";
        }
